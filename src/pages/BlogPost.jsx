    ...
    return (
        <div className="relative bg-gradient-to-b from-[#F4F9F1] to-[#EAEFE7] min-h-screen font-sans">
            ...
            <Helmet>
                ...
                <meta name="keywords" content={`${post.category?.name || 'organic farming'}, ${post.tags?.filter(tag => tag && tag.name).map(tag => tag.name).join(', ') || 'agriculture, Nepal, sustainable farming'}, Himalaya Krishi`} />
                ...
                {post.tags?.filter(tag => tag && tag.name).map((tag, index) => (
                ...
                <script type="application/ld+json">
                    {JSON.stringify({
                        ...
                        "keywords": post.tags?.filter(tag => tag && tag.name).map(tag => tag.name).join(', ') || 'organic farming, agriculture, Nepal'
                    })}
                </script>
            </Helmet>
            ...
            <div
                className="fixed top-0 left-0 h-1 bg-gradient-to-r from-[#1C4E37] via-[#D8A51D] to-[#1C4E37] z-50"
            ...
            <header className="pt-32 pb-20 px-4 relative">
                ...
                >
                    ...
                    <motion.h1
                        className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-[#1C4E37] mb-4 md:mb-6 tracking-tight leading-tight px-2 sm:px-0"
                        variants={heroTextAnimation}
                    >
                        {post.title}
                    </motion.h1>

                    <motion.div className="flex flex-wrap items-center gap-2 sm:gap-4 md:gap-6 mb-6 md:mb-8 px-2 sm:px-0" variants={heroTextAnimation}>
                        <span className="flex items-center px-2 sm:px-3 md:px-4 py-1 sm:py-2 bg-white/20 backdrop-blur-sm rounded-full text-[#3A5944] border border-white/30 text-xs sm:text-sm">
                        ...
                        {post.tags && post.tags.length > 0 && (
                            <span className="flex items-center px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-[#3A5944] border border-white/30">
                                ...
                                {post.tags.slice(0, 2).filter(tag => tag && tag.name).map(tag => tag.name).join(', ')}
                            </span>
                        )}
                    </motion.div>
                </motion.div>
            ...
            <main className="container mx-auto px-2 sm:px-4 max-w-5xl pb-16 md:pb-32">
                ...
                >
                    <div className="relative rounded-3xl overflow-hidden shadow-2xl group">
                        {featuredImage ? (
                            ...
                            <div className="w-full h-[300px] sm:h-[400px] md:h-[500px] lg:h-[600px] bg-gradient-to-br from-[#F4F9F1] via-white to-[#EAEFE7] flex items-center justify-center rounded-3xl shadow-inner border border-[#D8A51D]/20 relative overflow-hidden">
                                    ...
                                    {logoImage && (
                                        <div className="flex flex-col items-center justify-center p-8 h-full relative z-10">
                                            <motion.img
                                                ...
                                                className="h-32 sm:h-40 md:h-48 lg:h-56 w-auto mx-auto mb-4 sm:mb-6 md:mb-8 shadow-2xl rounded-2xl"
